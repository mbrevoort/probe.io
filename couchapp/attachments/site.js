$(document).ready(function() {
    var jqxhr = $.get('/api/browser', function(data) {
        ...
        $.each(data.rows, function(i, row) {
            ...
            stats[browserVersion] = stats[browserVersion] || {
                transports: {}
            };
            stats[browserVersion].transports[transportName] = row.value;
            if ((stats[browserVersion].total || 0) < row.value.count) stats[browserVersion].total = row.value.count;
        });
    ...
    jqxhr.error(function(error) {
        $('#stats').append('<div class="error">Error loading stats: ' + error + '</div>');
    });
});
...
function renderStats(stats) {
    ...
    $.each(keys(stats), function(i, browserKey) {
        var stat = stats[browserKey],
            data = [],
            series = [],
            numSeries = keys(stat.transports).length,
            TOTAL = 0,
            RTT = 1 * (numSeries + 1),
            SERIAL = 2 * (numSeries + 1),
            CONNECT = 3 * (numSeries + 1),
            middle = Math.floor(numSeries / 2),
            ticks = [
                [TOTAL + middle, "Total"],
                [RTT + middle, "RTT"],
                [SERIAL + middle, "Serial"],
                [CONNECT + middle, "Connect"]
            ],
        ...
        $.each(keys(stat.transports), function(j, transportKey) {
            var transport = stat.transports[transportKey];

            data = [
                [transport.total, TOTAL + j],
                [transport.rtt, RTT + j],
                [transport.serial, SERIAL + j],
                [transport.connect, CONNECT + j]
            ];
